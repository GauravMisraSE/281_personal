...
def upload_file():
    ...
    if request.method == 'POST':
            ...
            if datafile and allowed_file(datafile.filename):
                ...
                print ("current working directory is ", (check_output(['pwd'])))
                call(["java","-jar","CMPE202.jar","/home/ubuntu/281_personal/"+folder,"/home/ubuntu/281_personal/parser/"+folder])
...
def grader():
    ...
    # db.session.add(grades)
            #db.session.commit()
        return ('added remarks to database',{'Access-Control-Allow-Origin':'*'})
